...
function setLoding(){
 	...
 	var data = [];//全部的图片
 	var loadnum = 0;//已加载的图片数量
 	
 	for(var imgarr in imgData){
 		data = data.concat(imgData[imgarr]);
 	}
 	...
 	for(var i=0;i<data.length; i++){
 		...
 		img.onload = function(){
 			loadnum++;
 			logoText.innerHTML = "已加载 "+(Math.floor(loadnum/data.length*100))+"%";
 			
 			if(loadnum == data.length-1){
 				anmt();
 			}
 		}
 	}
...
function anmt(){
	...
	logo3 = document.createElement("div");
